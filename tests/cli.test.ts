...
describe('Execution tests', () => {
  ...
  it('No parameters provided', async () => {
    const result = await execute('node lib/src/cli.js migrate', '.');
  ...
  it('No migration directory', async () => {
    const command =
      "node ./lib/src/cli.js  migrate --host=http://sometesthost:8123 --user=default --password='' --db=analytics --migrations-home=/app/clickhouse/migrations";
  ...
  it('Environment variables are provided, but no migration directory', async () => {
    const result = await execute('node lib/src/cli.js migrate', { env: envVars });
  ...
  it('Incorrectly named migration', async () => {
    const command =
      "node ./lib/src/cli.js  migrate --host=http://sometesthost:8123 --user=default --password='' --db=analytics --migrations-home=tests/migrations/bad";
